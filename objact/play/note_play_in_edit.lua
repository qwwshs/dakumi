...
objact_note_play_in_edit = {
    draw = function(pos,istrack)
        ...
        local note_index = tonumber(string.sub(displayed_content,5,#displayed_content)) --选中的note
        if string.sub(displayed_content,1,4) == "note" and --选中note框绘制
            ...
            local event_index = tonumber(string.sub(displayed_content,6,#displayed_content)) --选中的event
            ...
            chart.event[event_index].track == track.track then --框出现在编辑的event
                ...
                love.graphics.rectangle("fill",pos,settings.judge_line_y + 10,track_info.w,800 - settings.judge_line_y) --遮罩
