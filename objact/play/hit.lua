...
local hit = love.graphics.newImage("asset/hit.png")
...
objact_hit = {
    ...
    draw = function()
        ...
        for i = 1,#hit_tab do
            ...
            local hit_scale_h = 1 / hit_h * 300 / (window_h_scale / window_w_scale)
            
            local hit_light_scale_w = 1 / hitlight_w
            local hit_light_scale_h = 1 / hitlight_h  / (window_h_scale / window_w_scale)
            if demo_mode then
                hit_scale_h = 1 / hit_h * 300 / (window_h_scale / window_w_scale)   / (1 / (1600/900))
            ...
            if demo_mode then
                ...
                ,hit_tab[i].x-150 *hit_alpha,settings.judge_line_y - 150 *hit_alpha / (window_h_scale / window_w_scale) / (1 / (1600/900)),0,hit_scale_w * hit_alpha,hit_scale_h * hit_alpha)
                
            else
                love.graphics.draw(hit
                ,hit_tab[i].x-150 *hit_alpha,settings.judge_line_y - 150 *hit_alpha  /(window_h_scale / window_w_scale),0,hit_scale_w* hit_alpha,hit_scale_h* hit_alpha)
            end
            ...
            if not (hit_light_alpha >= 19 or hit_light_alpha <= 1) then --防止暂停时滞留
                ...
                love.graphics.draw(hit_light,x,settings.judge_line_y-300  / (window_h_scale / window_w_scale),0,hit_light_scale_w*w,hit_light_scale_h*300)
            end
