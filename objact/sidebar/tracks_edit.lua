...
local function will_add()
    objact_message_box.message('add')
...
local function will_sub()
    objact_message_box.message(choose_track)
    if tracks_table[choose_track] then
        objact_message_box.message('delete')
...
objact_tracks_edit = {
    ...
    draw = function()
        ...
        love.graphics.print(objact_language.get_string_in_languages("track"),x,300)
        for i = 1,#tracks_table do
