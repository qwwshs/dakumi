...
object_alt_note_event = {
    keyboard = function(key)
        ...
        if key == 'b' then --翻转
            if string.sub(sidebar.displayed_content,1,5) == "event" and
            chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))] then
                log(chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))])
                chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].from = 100 - chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].from
                chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].to = 100 - chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].to
            end
        end
        if key == "t" then --快速调整
            if string.sub(sidebar.displayed_content,1,5) == "event" and
