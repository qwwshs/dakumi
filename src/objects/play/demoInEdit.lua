    ...
    if sidebar.displayed_content == "note" and --选中note框绘制
        ...
        for i=1,#chart.event do
            if chart.event[i].track == istrack then
                ...
                -- beizer曲线
                    for k = 1,10 do
                        local nowx = low_bezier(1,10,(chart.event[i].from - 50) / 100 * one_track_w + x_pos +one_track_w/2,x_pos + one_track_w/2 + (chart.event[i].to - 50) / 100 *one_track_w,chart.event[i].trans,k) --减去50是为了使50居中
