...
function hit:draw()
    ...
    local local_hit_tab = {}
    for i = 1,#self.tab do
        local hit_scale_w = 1 / self.hitW * 300
        local hit_scale_h = 1 / self.hitH * 300 / (WINDOW.scale / WINDOW.scale)
        
        local hit_light_scale_w = 1 / self.lightW
        local hit_light_scale_h = 1 / self.lightH  / (WINDOW.scale / WINDOW.scale)
        if demo then
            hit_scale_h = 1 / self.hitH * 300 / (WINDOW.scale / WINDOW.scale)   / (1 / (WINDOW.w/900))
            hit_scale_w = 1 / self.hitW * 300
        end
        love.graphics.setColor(1,1,1,1)
        local hit_alpha = math.floor((time.nowtime - self.tab[i].time) / 0.5 * 40) /40
        local hit_light_alpha = math.floor((time.nowtime - self.tab[i].time) / 0.5 * 19)+1
        ...
        if demo then
            ...
            ,self.tab[i].x-150 *hit_alpha,settings.judge_line_y - 150 *hit_alpha / (WINDOW.scale / WINDOW.scale) / (1 / (WINDOW.w/900)),0,hit_scale_w * hit_alpha,hit_scale_h * hit_alpha)
            
        else
            love.graphics.draw(self.hit
            ,self.tab[i].x-150 *hit_alpha,settings.judge_line_y - 150 *hit_alpha  /(WINDOW.scale / WINDOW.scale),0,hit_scale_w* hit_alpha,hit_scale_h* hit_alpha)
        end
        ...
        if not (hit_light_alpha >= 19 or hit_light_alpha <= 1) then --防止暂停时滞留
            ...
            love.graphics.draw(self.light,x,settings.judge_line_y-300  / (WINDOW.scale / WINDOW.scale),0,hit_light_scale_w*w,hit_light_scale_h*300)
        end
    end
