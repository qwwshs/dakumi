...
object_event_edit_bezier = { --bezier编辑
    load = function(x1,y1,r1,w1,h1)
        ...
        h = h1
        r = r1
        if not(string.sub(sidebar.displayed_content,1,5) == "event") then -- 太特殊 所以不用type 和roomtype 判断
            return
        end
        control_point = {x+w*chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].trans[1],
        y+h-chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].trans[2] *h,
        x+w*chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].trans[3],
        y+h-chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].trans[4] *h,
        }
    end,
    draw = function()
        if not(string.sub(sidebar.displayed_content,1,5) == "event") then -- 太特殊 所以不用type 和roomtype 判断
        ...
        for i = 1,100 do --曲线绘制
            ...
            y + h,
            y,
            chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].trans,
            i
            )
    ...
    update = function(dt)
        ...
        chart.event[tonumber(string.sub(sidebar.displayed_content,6,#sidebar.displayed_content))].trans = {trans1,trans2,trans3,trans4}
    end,
